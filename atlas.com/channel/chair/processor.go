...
func (p *Processor) Use(m _map.Model, chairType string, chairId uint32, characterId uint32) error {
	...
	return producer.ProviderImpl(p.l)(p.ctx)(EnvCommandTopic)(commandUseProvider(m, chairType, chairId, characterId))
}

func (p *Processor) Cancel(m _map.Model, characterId uint32) error {
	return producer.ProviderImpl(p.l)(p.ctx)(EnvCommandTopic)(commandCancelProvider(m, characterId))
}

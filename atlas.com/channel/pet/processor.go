...
func (p *Processor) Spawn(characterId uint32, petId uint64, lead bool) error {
	...
	return producer.ProviderImpl(p.l)(p.ctx)(EnvCommandTopic)(spawnProvider(characterId, petId, lead))
}
...
func (p *Processor) Despawn(characterId uint32, petId uint64) error {
	...
	return producer.ProviderImpl(p.l)(p.ctx)(EnvCommandTopic)(despawnProvider(characterId, petId))
}
...
func (p *Processor) AttemptCommand(petId uint64, commandId byte, byName bool, characterId uint32) error {
	...
	return producer.ProviderImpl(p.l)(p.ctx)(EnvCommandTopic)(attemptCommandProvider(petId, commandId, byName, characterId))
}
...
func (p *Processor) SetExcludeItems(characterId uint32, petId uint64, items []exclude.Model) error {
	...
	return producer.ProviderImpl(p.l)(p.ctx)(EnvCommandTopic)(setExcludesCommandProvider(characterId, petId, itemIds))
}
